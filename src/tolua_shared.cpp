...
int tolua_sharedcol(lua_State* L) {
	...
	std::shared_ptr<void>* p = *reinterpret_cast<std::shared_ptr<void>**>(lua_touserdata(L, -1));
	if (p) {
		delete p;
	}
	return 0;
...
void tolua_pushsharedusertype(lua_State* L, const std::shared_ptr<void>& ptr, const char* name) {
	...
	if (lua_isnil(L, -1)) {
		lua_pop(L, 1);
		lua_pushlightuserdata(L, ptr.get());
		*reinterpret_cast<void**>(lua_newuserdata(L,sizeof(void*))) = new std::shared_ptr<void>(ptr);
		lua_pushvalue(L, -1);
		lua_insert(L, -5);
		lua_rawset(L, -3);
		lua_pop(L, 1);
...
std::shared_ptr<void> tolua_tosharedusertype(lua_State* L, int index, const char* name) {
	...
	if (lua_getmetatable(L, index)) {
		...
		if (lua_rawequal(L, -1, -2)) {
			lua_pop(L, 2);
			std::shared_ptr<void>* p = *reinterpret_cast<std::shared_ptr<void>**>(lua_touserdata(L, index));
			if (p) {
				return *p;
			}
			return std::shared_ptr<void>();
